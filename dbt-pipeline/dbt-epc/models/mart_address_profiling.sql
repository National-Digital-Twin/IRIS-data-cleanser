...
with src as (
    select * from {{ ref('int_epc_address_dedup') }}
)
...
select
    ...
    null::text as "EnvironmentalImpactRatingBand",
    null::numeric as "FuelBills(£/yr)",
    null::numeric as "RealisticFuelBill(Regional)",
    tco2 as "tCO2",
    heating_cost_gbp_per_yr as "HeatingCost(£/yr)",
    saprating as "SAPRating",
    sapband as "SAPBand",
    lodgement_date as "LodgementDate",
    null::text as "SAPVersion",
    property_type as "PropertyType",
    construction_age_band as "ConstructionAgeBand",
    built_form as "BuiltForm",
    storeys_count as "StoreysCount",
    null::text as "FlatLocation",
    flat_level as "FlatLevel",
    null::numeric as "LowestFloorArea",
    main_heating_category as "MainHeatingCategory",
    main_fuel_type as "MainFuelType",
    main_heating_control as "MainHeatingControl",
    null::text as "DerivedSAPMainHeatingCode",
    null::text as "HeatEmitterType",
    null::text as "WaterHeatingFuel",
