...
```
make run-sonar-scan REPOSITORY=frontend SONAR_PROJECT_KEY=test SONAR_TOKEN=test SOURCE_CODE_DIR_AND_FILES=test TEST_DIR_AND_FILES=test REPORT_PATHS=test/test.xml
```
...
The SOURCE_CODE_DIR_AND_FILES is a comma separated list of files and directories which contain all the source code to be scanned.
The TEST_DIR_AND_FILES is a comma separated list of the files and directories which contain the tests for the repository you wish to scan. Please remove this parameter if not required.
The REPORT_PATHS is a comma separated list of coverage reports generated by your tools of choice. Please remove this parameter if not required in the make file.
